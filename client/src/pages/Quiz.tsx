...
const steps = [
  ...
  // 13: Story (Info)
  {
    ...
    icon: <Battery className="w-24 h-24 text-red-500 mx-auto mb-6" />
  },
  ...
  // 20: Expert Review (Info)
  {
    ...
    icon: <Brain className="w-24 h-24 text-primary mx-auto mb-6" />
  },
        ...
        >
          ...
          {currentStep.type === "info" && (
            <div className="text-center space-y-8 py-8">
              {currentStep.icon}
              {currentStep.visual === "cards" && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                  {["Behavioral Science", "Skill Acquisition", "Habit Engineering"].map((text) => (
                    <div key={text} className="p-4 bg-white rounded-xl shadow-sm border border-border text-sm font-semibold text-primary">
                      {text}
                    </div>
                  ))}
                </div>
